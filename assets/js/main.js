...
function addToCart(id,qty=1){const p=products.find(x=>x.id===Number(id));if(!p)return;const c=getCart(),i=c.find(x=>x.id===p.id);if(i)i.qty=Math.min(99,i.qty+qty);else c.push({...p,qty:Math.max(1,qty)});saveCart(c);toast(p.name+' added to cart')}
...
function card(p){const w=getWishlist().includes(p.id);return `<article class="product-card reveal"><a class="product-img" href="${productLink(p.id)}">${productVisual(p)}<span class="product-tag">${escapeHtml(p.tag||'')}</span></a><div class="product-info"><p>${escapeHtml(p.category)}</p><h3><a href="${productLink(p.id)}">${escapeHtml(p.name)}</a></h3><div class="price-row"><span class="price">${money(p.price)}</span>${p.old?`<span class="old">${money(p.old)}</span>`:''}</div><div class="card-actions"><button class="mini-btn" onclick="addToCart(${p.id})">Add to Cart</button><button class="wish-btn ${w?'active':''}" aria-label="Wishlist" onclick="toggleWishlist(${p.id})">${w?'♥':'♡'}</button></div></div></article>`}
function renderFeatured(){const e=document.getElementById('featuredProducts');if(e)e.innerHTML=products.slice(0,4).map(card).join('')}
